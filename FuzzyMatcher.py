...
class FuzzyMatcher(object):
    ...
    def get_match_factor(file_type, file_artist, file_title, serv_artist, serv_title):
        if file_type < 2 or file_artist == file_title:
            return compare_single(file_artist, serv_artist, serv_title)
        else:
            return compare_duos(file_artist, file_title, serv_artist, serv_title)
...
def compare_duos(file_artist, file_title, serv_artist, serv_title):
    ...
    if file_title == serv_title or file_title == serv_artist:
        if check_substrings(file_artist, serv_artist) or check_substrings(file_artist, serv_title):
            return 0.9

    if file_artist == serv_title or file_artist == serv_artist:
        if check_substrings(file_title, serv_artist) or check_substrings(file_title, serv_title):
            return 0.9

    # Case 3 : Neither is an exact match but each is at least present as a substring.
    if (check_substrings(file_artist, serv_artist) or check_substrings(file_artist, serv_title)) and \
            (check_substrings(file_title, serv_artist) or check_substrings(file_title, serv_title)):
        return 0.7

    # Worst Possible Case : At least one of them is present as a substring in either the artist/title..
    if (check_substrings(file_artist, serv_artist) or check_substrings(file_artist, serv_title)) or \
            (check_substrings(file_title, serv_artist) or check_substrings(file_title, serv_title)):
        return 0.4
